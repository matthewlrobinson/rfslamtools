...
```{r}
rpart_summary(df_with_predictions,'p.hat',vars_list)
```
...
```{r}
calibration_plot(df_with_predictions, target = "label", risk_col = 'p.hat', vars_list) 
```
Or use one function to display all relevant plots
```{r}
analysis_plots(df_with_predictions, "label", "pid", "p.hat", "cpiu", analysis_vars)
```
